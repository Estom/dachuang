...
while True:
    ...
    while True:
        ...
        if (state[1] % 2**2)/2**0 % 2 == 1:
            ...
            num += 1
            info = SQLconfig.sql1.select('dcweb_article', ['id', 'title', 'content'], None, 1, index)
    ...
    for ii in range(numcontent):
        ...
        SQLconfig.sql1.update('dcweb_article', {'tag_mark': Data_ID[ii][1] % 2 ** 2 + 2 ** 0}, 'id = %d' % Data_ID[ii][0])
